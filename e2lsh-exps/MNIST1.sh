...
./alsh -A fraction_recall_e2kl -n ${N} -q ${QN} -d ${DD} -D ${dataset} -Q ${query} -W ../data/Mnist784/Mnist784_normal.w -G ${groundtruth} -O ${OUTPUT} -U 3.140000 --data_hash_filename ${DATA_HASH}  --query_hash_filename ${QUERY_HASH} -r 0.5